...
final class BookmarkRepository: BookmarkRepositoryType {
    ...
    func fetchedResultsController(isFavorite: Bool = false) -> NSFetchedResultsController<BookmarkEntity> {
        ...
        let context = isFavorite ? backgroundContext : viewContext
    ...
    func update(bookmark: Bookmark) -> Result<Void, Error> {
        ...
        request.predicate = predicate
        do {
            guard let bookmarkEntity = try backgroundContext.fetch(request).first else { return .success(()) }
    ...
    func updateFavorites(for url: URL) -> Result<Bool, Error> {
        ...
        request.predicate = predicate
        do {
            guard let bookmarkEntity = try backgroundContext.fetch(request).first else { return .success(false) }
    ...
    func updateTags(_ tag: Tag, to newTag: Tag) -> Result<Void, Error> {
        ...
        request.predicate = predicate
        do {
            let bookmarkTagEntities = try backgroundContext.fetch(request)
    ...
    func delete(for url: URL) -> Result<Void, Error> {
        ...
        do {
            guard let bookmarkEntity = try backgroundContext.fetch(request).first else { return .success(()) }
            backgroundContext.delete(bookmarkEntity)
            try backgroundContext.save()
            return .success(())
        } catch {
            return .failure(error)
    ...
    func deleteTags(for tag: Tag) {
        ...
        do {
            let bookmarkTagEntities = try backgroundContext.fetch(request)
            bookmarkTagEntities.forEach { backgroundContext.delete($0) }
            try backgroundContext.save()
        } catch {
            return
        }
